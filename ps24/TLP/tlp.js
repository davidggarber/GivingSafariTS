...
function isEmpty(span) {
  if (!span) {
    return false;
  }
  var yx = span.id.substring(1).split('_').map(n => parseInt(n));
  var color = mazeColors[yx[0]][yx[1]];
  if (color != ' ') {
    return false;
  }
  if (!hasClass(span, 'cleared-span')) {
    var obj = mazeText[yx[0]][yx[1]];
    if ((obj >= 'A' && obj <= 'Z') || (obj >= '1' && obj <= '9')) {
      return false;
    }
  }
  return true;
...
function fightMonster(ch) {
  ...
  var newLine = list.innerHTML == ''
    || list.innerHTML.endsWith('<br></span>');
  var span = document.createElement('span');  
  span.innerHTML = (newLine ? '' : ', ') 
    + '<b>' + monster.name[0] + '</b>' + monster.name.substring(1);
  if (!newLine && monster.hp >= 10) {
...
function spanInDir(dx, dy, shift) {
  var i = 1;
  var step = spanAt(playerPos.x + dx * i, playerPos.y + dy * i);
  var slide = step;
  while (shift && slide && isEmpty(slide)) {
    step = slide;
    i++;
    slide = spanAt(playerPos.x + dx * i, playerPos.y + dy * i);
  }
  return step;  // first step, if can't walk to, otherwise keep going
}
...
function onArrowKey(evt) {
  ...
  if (playerChar && playerHP > 0) {
    if (evt.code == 'ArrowLeft' || evt.code == 'KeyA') {
      dest = spanInDir(-1, 0, evt.shiftKey || evt.ctrlKey);
    }
    else if (evt.code == 'ArrowRight' || evt.code == 'KeyD') {
      dest = spanInDir(1, 0, evt.shiftKey || evt.ctrlKey);
    }
    else if (evt.code == 'ArrowUp' || evt.code == 'KeyW') {
      dest = spanInDir(0, -1, evt.shiftKey || evt.ctrlKey);
    }
